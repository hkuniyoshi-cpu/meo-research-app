...
function ctaPersonal(d) {
  const t = d.profile.total;
  let head, sub, cls;
  if (t < 60) { cls = "high"; head = "今が動きどきです。"; sub = `現在 <b>${t}点</b>。検索で「見つけてもらう機会」を逃しているサインです。正しいMEO対策で巻き返せます。`; }
  else if (t < 80) { cls = "mid"; head = "あと一歩で上位圏。"; sub = `<b>${t}点</b>は惜しい水準。プロの仕上げで、競合との差がはっきりつきます。`; }
  else { cls = "low"; head = "この強みを、独走に。"; sub = `<b>${t}点</b>は上位水準。維持・拡大には継続的な運用が鍵。さらに上を狙えます。`; }
  return `<div class="cta-card cta-${cls}">
    <div class="cta-head">${head}</div>
    <p class="cta-sub">${sub}</p>
    <a class="cta-btn" href="${CTA.site}" target="_blank" rel="noopener">この結果を無料でプロに相談する</a>
    <div class="cta-by">MEO/SEOのプロ集団 <b>SearchMania</b> が、最短ルートをご提案します</div>
  </div>`;
}
// B. 競合差を突くCTA（1位以外のとき）
function ctaGap(d) {
  ...
  return `<div class="cta-gap">
    ...
    <div>近隣同業${d.ranking.total}店中 <b>${d.ranking.rank}位</b>。${gap ? `1つ上の店との知名度差は <b>${gap}pt</b>。` : ""}この差は<b>正しいMEO対策で詰められます</b>。
    <a href="${CTA.site}" target="_blank" rel="noopener">差を埋める方法を相談する →</a></div>
  </div>`;
}
// C. 自分でやる vs プロに任せる
function ctaVs() {
  return `<div class="glass vs-card">
    <div class="g-head"><span class="g-ico">🤔</span>自分で対策 vs プロに任せる</div>
    <table class="vs"><thead><tr><th></th><th>自分で</th><th class="vs-pro">SearchMania</th></tr></thead><tbody>
      <tr><td>時間・手間</td><td>大（試行錯誤）</td><td class="vs-pro">おまかせ</td></tr>
      <tr><td>専門知識</td><td>学習が必要</td><td class="vs-pro">不要</td></tr>
      <tr><td>継続運用</td><td>続けにくい</td><td class="vs-pro">代行で継続</td></tr>
      <tr><td>成果</td><td>不確実</td><td class="vs-pro">実績に基づく</td></tr>
    </tbody></table></div>`;
}
// D. 末尾の強い相談セクション
function ctaFinal() {
  const line = CTA.line ? `<a class="fc-btn fc-line" href="${CTA.line}" target="_blank" rel="noopener">LINEで相談</a>` : "";
  const tel = CTA.tel ? `<a class="fc-btn fc-tel" href="tel:${CTA.tel}">電話で相談</a>` : "";
  return `<div class="glass final-cta">
    <div class="fc-head">この診断結果、<br>プロと一緒に改善しませんか？</div>
    <p class="fc-sub">SearchManiaが「何から手をつけるべきか」を<b>無料</b>でご提案します。</p>
    <div class="fc-benefits"><span>✓ 専門知識は不要</span><span>✓ 時間をかけずに</span><span>✓ 成果にこだわる</span></div>
    <div class="fc-btns">
      <a class="fc-btn fc-primary" href="${CTA.site}" target="_blank" rel="noopener">無料相談・お問い合わせ</a>
      ${line}${tel}
      <a class="fc-btn fc-mail" href="mailto:${CTA.email}?subject=MEO診断の相談">メールで相談</a>
    </div>
  </div>`;
}
